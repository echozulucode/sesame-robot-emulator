...
/*
  The scans that decide whether a claim is readable — Phase 5 T5.

  They used to be declared inside phases 12 and 13, which is why phase 13 kept
  its own restated copy of the correctness-selector list. `honesty-probes.mjs`
  is the scope all three callers share, and the third caller is phase 14: the
  PACKAGED artefact has to be measured with the same contrast arithmetic, the
  same hit-tested target boxes and the same 14 px floor as the dev build, not
  with a second implementation of them.
*/
import {
  CONTRAST_JS,
  CONTRAST_LARGE_TEXT,
  ...
  TEXT_FLOOR_PX,
  TYPE_SCAN_JS,
} from './lib/honesty-probes.mjs';
import { attachToDebugPort } from './lib/cdp.mjs';
...
async function launchBrowser(url, window = DEFAULT_WINDOW) {
  ...
  /*
    The socket, the id correlation and the page-error collector moved to
    `scripts/lib/cdp.mjs` in Phase 5 T5, unchanged.
  ...
  */
  const session = await attachToDebugPort({
    port: cdpPort,
    // This browser is given its URL on the command line, so any page target is
    // the right one; the packaged phase is the caller that must be pickier.
    accept: (t) => t.type === 'page' && typeof t.webSocketDebuggerUrl === 'string',
  }).catch((e) => die(`the browser never exposed a CDP page target: ${e.message}\n${stderr}`));
  const { cdp, evaluate, errors: pageErrors, close: closeSocket } = session;
  ...
  */
  return { evaluate, cdp, shoot, close, stderr: () => stderr, errors: pageErrors, target: session.target };
}
...
// ===========================================================================
{
  ...
  /*
    The W1 type scan, and the list of surfaces it must find whole, live in
    `scripts/lib/honesty-probes.mjs` — phase 13 and the packaged phase ask the
    same questions, and a second copy of them is a second answer.
  */
  const CORRECTNESS_SELECTORS = CORRECTNESS_SURFACE_SELECTORS;
  const typeScan = (evaluate) => evaluate(TYPE_SCAN_JS);
...
// ===========================================================================
{
  ...
  /*
   * Phase 12's list, SHARED rather than restated — Phase 5 T5.
   *
   * This was a second copy, under a comment calling the duplication "the
   * smaller evil" because there was no scope in which both phases could see one
   * array. `scripts/lib/honesty-probes.mjs` is that scope now, and the packaged
   * phase would otherwise have been the third copy.
   */
  const W6_CORRECTNESS_SELECTORS = CORRECTNESS_SURFACE_SELECTORS;

  // The two target tiers, the WCAG floor under them and the three contrast
  // ratios are imported from scripts/lib/honesty-probes.mjs, together with the
  // scans that interpolate them.
  ...
  // CONTRAST_JS — scripts/lib/honesty-probes.mjs

  // TARGETS_JS — scripts/lib/honesty-probes.mjs

  // OVERFLOW_JS — scripts/lib/honesty-probes.mjs

  // CORRECTNESS_JS — scripts/lib/honesty-probes.mjs

  // PROVENANCE_JS — scripts/lib/honesty-probes.mjs

  // FOCUSABLE_SEL — scripts/lib/honesty-probes.mjs

  // FOCUS_STATE_JS — scripts/lib/honesty-probes.mjs

  // ENUM_FOCUSABLE_JS — scripts/lib/honesty-probes.mjs

  // POINTER_ONLY_JS — scripts/lib/honesty-probes.mjs
...
// ===========================================================================
// PHASE 14: THE HONESTY SURFACES, IN THE PACKAGED ARTEFACT — Phase 5 T5
// ===========================================================================
//
// Everything above this line was measured in headless Chromium against
// `apps/web/dist` served over HTTP. None of it says anything about the
// executable a child double-clicks, and packaging is exactly where the
// provenance work would be quietly lost — a CSP that strips something, a build
...
//
// It reports `ran: false` WITH A REASON and the command that fixes it when
// there is no artefact. It is never silently skipped, and it never reports
// success without having driven a window: the first thing it does after
// attaching is check the CDP target says `tauri.localhost`, because a plain
// `cargo build --release` serves `devUrl` and would have this whole phase
// grading a Vite dev server.
//
// The implementation is `scripts/lib/packaged-honesty.mjs` — it has different
// prerequisites from everything else here and keeping it separable is what
// lets `just tauri-honesty` run it alone.
phases.packagedHonesty = await runPackagedHonestyPhase({
  repo: REPO,
  check,
  note: (text) => notes.push(text),
  log: console.log,
  problemCount: () => problems.length,
  skip: argv.includes('--skip-packaged'),
  exePath: argOf('packaged-exe', null),
  shoot: (name, caption, buffer) => {
    fs.mkdirSync(OUT, { recursive: true });
    const file = path.join(OUT, name);
    fs.writeFileSync(file, buffer);
    const record = {
...
console.log(
  // "real-browser" was true of all of them until T5. Three now come out of the
  // PACKAGED desktop window instead, which is a real browser engine but not the
  // headless Chromium the rest of this sentence is about, and a summary line
  // that quietly absorbed the difference would be the same species of
  // overclaim the phase it describes exists to prevent.
  `OK    ${shots.length} captures — ${shots.length - (phases.packagedHonesty?.ran === true ? 3 : 0)} ` +
    `in a headless browser` +
    (phases.packagedHonesty?.ran === true ? ' and 3 in the PACKAGED desktop window' : '') +
    `; joint rotations read back from the scene graph; ` +
